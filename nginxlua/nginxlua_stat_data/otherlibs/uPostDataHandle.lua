                     ...
                     local ok,err = red:connect("10.2.0.4",6379)
                     if not ok then
                     ...
                     local ok,err = redata:connect("10.2.0.4",6379)
                     if not ok then
...
function ServiceRouteInterface(vid,pid)
    ...
    ---[[
    local res1,err = redata:get("custom_collinfo_uid")
    if res1 ~= ngx.null then
       table1 = cjson.decode(res1)
       set1 = Set(table1)
       if set1[uid] then
...
function RealHandleCollInfo(vid,uid,pid)    
    if CheckKey("redata",vid.."_"..pid.."_".."S") and CheckKey("redata",vid.."_"..pid.."_".."z1") then
       ...
       if res1 ~= ngx.null and res2 ~= ngx.null then
          ...
          local ok,err = redata:rpush(uid.."_collinfo_list",tjson)
          if not ok then
