                     ...
                     --   write by wye
                     --   Copyright @ 2013 - 2014  Cloudiya Tech . Inc
                     -- /
                     ...
                     if not ok then
                        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china - Fail connect to local redis , Error info "..err)
                        return
                     end

                     -- Connect To 10.2.10.19:6380
                     local redata = redis:new()
                     redata:set_timeout(1000)
                     local ok,err = redata:connect("10.2.10.19",6380)
                     if not ok then
                        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china - Fail connect to remote redis , Error info "..err)
                        return
                     end
                     
                     -- Connect To 10.2.10.19:6379 For Video Meta Data
                     local redatav = redis:new()
                     redatav:set_timeout(1000)
                     local ok,err = redatav:connect("10.2.10.19",6379)
                     if not ok then
                        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china - Fail connect to remote redis , Error info "..err)
                        return
                     end                     
...
function RealHandleCollInfo(vid,uid,pid)    
    if CheckKey("redata",vid.."_"..pid.."_".."S") and CheckKey("redata",vid.."_"..pid.."_".."z1") then
       ...
       if res1 ~= ngx.null and res2 ~= ngx.null then
          ...
          z1table["comprate"] = Stable["comprate"]
 ...
 function PlayerLoadFail(key,value)
    ...
    local time = os.time() 
    local uos,browser = ParseOSandBrowser(useragent)
 ...
 -- vid_pid_Y
 function CheckFlow(key,value,vid,pid,tid)
    ...
    if value["region"] == nil then
       value["region"] = "unknown"
    end
    if value["city"] == nil then
       value["city"] = "unknown"
    end                   
    
    --Real-time statistics tid loadnum
    local ok,err = redata:hincrby(tid.."_converge","loadnum",1)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--CheckFlow--Fail hincrby to redata,Error info"..err)
       return
    end

    --Statistics tid loadnum daily
    loadtime = os.date("%Y%m%d")
    local ok,err = redata:hincrby(tid.."_loadnumdaily_ht",loadtime,1)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--CheckFlow--Fail hincrby to redata,Error info"..err)
       return
    end

    --Real-time statistics tid ads place home
    referer = value["dm"]
    local ok,err = redata:hincrby(tid.."_referer",referer,1)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--CheckFlow--Fail hincrby to redata,Error info"..err)
    end
    
    -- Save tid_pid_Y to red 
    jsonvalue = cjson.encode(value)
    local ok,err = red:set(key,jsonvalue)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--CheckFlow--Fail set to red,Error info "..err)
       return
    end
    local ok,err = red:expire(key,129600)
 ...
 function PlayVideoFail(key,value)
    ...
    local ok,err = red:set(key,jsonvalue)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayVideoFail--Fail set to redis,Error info "..err)
       return
    end
    local ok,err = red:expire(key,129600)
 end
 ...
 function PlayVideoSuc(key,value)
    ...
    a,b,tid,pid,flag = string.find(key,"(.*)_(.*)_(.*)")
    red:zadd(tid.."_"..pid.."_set",tonumber(flag),jsonvalue)
    local ok,err = red:expire(key,129600)
    local ok,err = red:expire(tid.."_"..pid.."_set",129600)
    
    -- Write tid_pid to pendinglist in red for prepared to handle's tid_pid
    --local ok,err = red:rpush("pendinglist",tid.."_"..pid)
    local ok,err = red:zadd("pendingset",os.time(),tid.."_"..pid)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayVideoSuc--Fail rpush to redata,Error info"..err)
       return
    end
 end
...
function PlayWindowClose(tid,pid,Cvalue)

    local vid = string.sub(tid,1,7) 

    local ok,err = red:exists(tid.."_"..pid.."_".."set")
    if not ok then
        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayWindowClose--Fail connect to redis server :".." red ".." ,Error info "..err)
        return
    elseif ok == 0 then
        succ, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayWindowClose--Not find key : "..tid.."_"..pid.."_".."set")
        return
    end

    -- Obtain "loadtime,ip,os,browser,country,region,city" from "tid_pid_Y"
    -- Obtain "starttime" from "tid_pid_0"
    red:init_pipeline()
    red:get(tid.."_"..pid.."_".."Y")
    red:get(tid.."_"..pid.."_".."0")
    local results,err = red:commit_pipeline()
    if not results then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayWindowClose--1--Fail get from redis pipeline,Error info "..err)
       return
    end                        
                        
    -- "S" will be store in redis(redata) by keyname "tid_pid_S"
    local S = cjson.decode(results[1])
    S["starttime"] = cjson.decode(results[2])["starttime"]
                        
    -- ########################
    -- Obtain video meta information from redatav , e.g Duration, avg bitrate every flow level.                        
    local ok,err = redatav:exists(vid.."_info")
    if not ok then
        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--CheckKey--Fail connect to redis server,Error info "..err)
    ...
    elseif ok == 0 then
        ...
        duration = 10000
    elseif ok == 1 then
        flbit_str = redatav:get(vid.."_info")
    ...
    -- ########################
                        ...
                        results = {}
                        the_date = red:zrange(tid.."_"..pid.."_set",0,-1)
                        ...
                        if htgetn(dtmplist) == 1 or htgetn(ftmplist) == 2 then
                            ...
                            if Cvalue["flag"] then
                               ...
                               local res,err = red:get(tid.."_"..pid.."_P")
                        ...
                        --Real-time statistics tid flow
                        redata:hincrby(tid.."_converge","currentQuota",flowsum)
                        redata:hincrby(tid.."_converge","currentQuotaTotal",flowsum)
                        ...
                        redata:set(tid.."_"..pid.."_".."S",cjson.encode(S))
                        redata:set(tid.."_"..pid.."_".."J",cjson.encode(periodnumlist))

                        local results,err = redata:commit_pipeline()
                        if not results then
                           succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayWindowClose--5--Fail set data to redata,Error info "..err)
                           return
                        end 
                        local ok,err = redata:expire(tid.."_"..pid.."_".."S",129600)
                        local ok,err = redata:expire(tid.."_"..pid.."_".."J",129600)
                        --####################### 
                        
                        --If handle success,move vid_pid from pendinglist to endlist
                        --ok,err = red:lrem("pendinglist",1,tid.."_"..pid)
                        local ok,err = red:zrem("pendingset",tid.."_"..pid)
                        if tonumber(ok) == 1 then
                           redata:rpush("endlist",tid.."_"..pid)
                        else
                           succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayWindowClose--6--Don't find the tid_pid in pendinglist,Error info")
                           return
                        end
                        
                        -- Increase 1 to vid in playtable in redata for statistics the video play numbers
                        local ok,err = redata:hincrby("playtable",tid,1)
                        if not ok then
                           succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--PlayWindowClose--Fail hincrby to redata,Error info"..err)
                           return
                        end
 ...
 function RecPlayInfo(key,value)
    ...
    local ok,err = red:set(key,jsonvalue)
    if not ok then
        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--RecPlayInfo--Fail set to redis,Error info "..err)
        return
    end
    local ok,err = red:expire(key,129600)
 end
 ...
 function VStreamSwitch(key,value)
    ...
    local ok,err = red:zadd(tid.."_"..pid.."_set",tonumber(Num),jsonvalue)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--VStreamSwitch--Fail set to redis,Error info "..err)
       return
    end
    local ok,err = red:expire(key,129600)
 end
 ...
 function VideoPlayError(key,value)
    ...
    local ok,err = red:set(key,jsonvalue)
    if not ok then
       succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--VideoPlayError--Fail set to redis,Error info "..err)
       return
    end
    local ok,err = red:expire(key,129600)
 end
 ...
 -- /
                     ...
                     if htgetn(args) == 2 then
                         ...
                         a,b,tid,pid,flag = string.find(args["key"],"(.*)_(.*)_(.*)")

                          -- The player no playlist 
                          -- Example : tid == "l123Xur13Jkl" , vid == "l123Xur"
                          if string.len(tid) == 12 and string.len(pid) == 8 then
                                                         
                             vid = string.sub(tid,1,7)                           
                             ...
                             -- Check user flow
                             if flag == "Y" then
                                CheckFlow(args["key"],tablevalue,vid,pid,tid)
                             end
                             ...
                             if flag == "C" then
                                RecPlayInfo(args["key"],tablevalue)
                                if CheckKey("red",tid.."_"..pid.."_".."Y") and CheckKey("red",tid.."_"..pid.."_".."0")  then
                                   PlayWindowClose(tid,pid,tablevalue)
                                end
                             end
                             
                             -- Tid Click Statistics
                             if flag == "click" then
		                        local ok,err = redata:hincrby(tid.."_converge","clicknum",1)
		                        if not ok then
		                           succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Fun--main--Fail hincrby to redata,Error info"..err)
		                           return
		                        end
                             end                             
                             ...
                             succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Main--"..args["key"].." data format is incorrect")
                             return
                          end                        
                           
                     else
                          succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Main--the number of parameters be 2,but it is "..htgetn(args))
                          return
                     end
                     ...
                     if not ok then
                        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Main--Fail to set red keepalive,error info : "..err)
                     end
                     local ok,err = redata:set_keepalive(10,100)
                     if not ok then
                        succ, err, forcible = log_dict:set(os.date("%x/%X"),"68china -- Main--Fail to set redata keepalive,error info : "..err)
                     end
